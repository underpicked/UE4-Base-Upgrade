...
// This is the main rendering function that you have to implement and provide to Gui (via setting up 'RenderDrawListsFn' in the GuiIO structure)
// If text or lines are blurry when integrating Gui in your engine:
// - in your Render function, try translating your projection matrix by (0.5f,0.5f) or (0.375f,0.375f)
void Gui_ImplDX11_RenderDrawLists(ImDrawData* draw_data)
{
	...
	// Setup orthographic projection matrix into our constant buffer
	{
		...
		float L = 0.0f;
		float R = Gui::GetIO().DisplaySize.x;
		float B = Gui::GetIO().DisplaySize.y;
		float T = 0.0f;
		float mvp[4][4] =
		{
	...
	vp.Width = Gui::GetIO().DisplaySize.x;
	vp.Height = Gui::GetIO().DisplaySize.y;
	vp.MinDepth = 0.0f;
	vp.MaxDepth = 1.0f;
...
// You can read the io.WantCaptureMouse, io.WantCaptureKeyboard flags to tell if dear Gui wants to use your inputs.
// - When io.WantCaptureMouse is true, do not dispatch mouse input data to your main application.
// - When io.WantCaptureKeyboard is true, do not dispatch keyboard input data to your main application.
// Generally you may always pass all inputs to dear Gui, and hide them from your application based on those two flags.
...
static void Gui_ImplDX11_CreateFontsTexture()
{
	// Build texture atlas
	GuiIO& io = Gui::GetIO();
...
bool Gui_ImplDX11_CreateDeviceObjects()
{
	if (!g_pd3dDevice)
		return false;
	if (g_pFontSampler)
		Gui_ImplDX11_InvalidateDeviceObjects();

	// By using D3DCompile() from <d3dcompiler.h> / d3dcompiler.lib, we introduce a dependency to a given version of d3dcompiler_XX.dll (see D3DCOMPILER_DLL_A)
	// If you would like to use this DX11 sample code but remove this dependency you can: 
	//  1) compile once, save the compiled shader blobs into a file or source code and pass them to CreateVertexShader()/CreatePixelShader() [preferred solution]
	//  2) use code to detect any version of the DLL and grab a pointer to D3DCompile from the DLL. 
	// See https://github.com/ocornut/Gui/pull/638 for sources and details.
	...
	Gui_ImplDX11_CreateFontsTexture();

	return true;
}

void    Gui_ImplDX11_InvalidateDeviceObjects()
{
	if (!g_pd3dDevice)
		return;

	if (g_pFontSampler) { g_pFontSampler->Release(); g_pFontSampler = NULL; }
	if (g_pFontTextureView) { g_pFontTextureView->Release(); g_pFontTextureView = NULL; Gui::GetIO().Fonts->TexID = NULL; } // We copied g_pFontTextureView to io.Fonts->TexID so let's clear that as well.
	if (g_pIB) { g_pIB->Release(); g_pIB = NULL; }
	if (g_pVB) { g_pVB->Release(); g_pVB = NULL; }
...
bool    Gui_ImplDX11_Init(void* hwnd, ID3D11Device* device, ID3D11DeviceContext* device_context)
{
	g_hWnd = (HWND)hwnd;
	g_pd3dDevice = device;
	...
	GuiIO& io = Gui::GetIO();
	io.KeyMap[GuiKey_Tab] = VK_TAB;                       // Keyboard mapping. Gui will use those indices to peek into the io.KeyDown[] array that we will update during the application lifetime.
	io.KeyMap[GuiKey_LeftArrow] = VK_LEFT;
	io.KeyMap[GuiKey_RightArrow] = VK_RIGHT;
	io.KeyMap[GuiKey_UpArrow] = VK_UP;
	io.KeyMap[GuiKey_DownArrow] = VK_DOWN;
	io.KeyMap[GuiKey_PageUp] = VK_PRIOR;
	io.KeyMap[GuiKey_PageDown] = VK_NEXT;
	io.KeyMap[GuiKey_Home] = VK_HOME;
	io.KeyMap[GuiKey_End] = VK_END;
	io.KeyMap[GuiKey_Insert] = VK_INSERT;
	io.KeyMap[GuiKey_Delete] = VK_DELETE;
	io.KeyMap[GuiKey_Backspace] = VK_BACK;
	io.KeyMap[GuiKey_Enter] = VK_RETURN;
	io.KeyMap[GuiKey_Escape] = VK_ESCAPE;
	io.KeyMap[GuiKey_A] = 'A';
	io.KeyMap[GuiKey_C] = 'C';
	io.KeyMap[GuiKey_V] = 'V';
	io.KeyMap[GuiKey_X] = 'X';
	io.KeyMap[GuiKey_Y] = 'Y';
	io.KeyMap[GuiKey_Z] = 'Z';

	io.RenderDrawListsFn = Gui_ImplDX11_RenderDrawLists;  // Alternatively you can set this to NULL and call Gui::GetDrawData() after Gui::Render() to get the same ImDrawData pointer.
	io.ImeWindowHandle = g_hWnd;

	return true;
}

void Gui_ImplDX11_Shutdown()
{
	Gui_ImplDX11_InvalidateDeviceObjects();
	Gui::Shutdown();
	g_pd3dDevice = NULL;
...
void Gui_ImplDX11_NewFrame()
{
	...
	// Hide OS mouse cursor if Gui is drawing it
